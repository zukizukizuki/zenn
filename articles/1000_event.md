---
title: "1日限り1000人規模イベントのインフラをワンオペで完遂した話"
emoji: "🤔"
...
## はじめに
参画先の企業で**「1日限定・来場者1000人規模のイベント」**を開催することになりました。
そこで
- インフラ
- CI/CD
- セキュリティ周りの設計・構築
- 監視設計
をまるっと任せていただいたので、どういう考えで構成し、どう乗り越えたかを振り返ってみます。
...
- **ミッション**
  ...
  インフラだけでなく、CI/CDパイプラインやセキュリティ設計まで一貫して担当しました。
...
flowchart TD
  ...
  ECS -.-> ECRApp[ECR (app/web)<br/>スキャンon push]
  ECS --> NAT[NAT Gateway] --> Internet
  RDS --> Backup[バックアップ保持<br/>環境別日数]
```
...
- **DBのスケーラビリティ**
  トラフィックが読みきれないイベントだったので、Aurora Serverless v2 などの選択肢をありだったと思います。
